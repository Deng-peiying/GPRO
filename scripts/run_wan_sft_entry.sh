#!/bin/bash
# ── B: Entry-focused SFT ────────────────────────────────────────────────
# I2V training on arm-entry windows. Targets embodiment-level failure repair.
set -euo pipefail

PROJECT_ROOT="${PROJECT_ROOT:-/root/autodl-tmp/main_exp/EVA-main_v0.5}"
cd "$PROJECT_ROOT"

DATA_ROOT="${DATA_ROOT:-/home/zhangmohan/data/robotwin_sft_entry}"
...
torchrun --nproc_per_node="$NPROC" --master_port="$MASTER_PORT" main.py \
    ...
    dataset=robotwin_sft_entry \
    algorithm=wan_i2v \
    +name=entry_sft_$(date +%Y%m%d_%H%M) \
    \
